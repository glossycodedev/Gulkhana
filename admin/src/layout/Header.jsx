...
const Header = ({showSidebar, setShowSidebar}) => {
  ...
  const {userInfo } = useSelector(state => state.auth)
    ...
    return (
        <div className='fixed top-0 left-0 w-full shadow-md  z-40'>
          <div className='ml-0 lg:ml-[260px]  h-[65px] flex justify-between items-center bg-[#ffffff] px-5 transition-all'>

        <div onClick={() => setShowSidebar(!showSidebar)} className='w-[35px] flex lg:hidden h-[35px] rounded-sm bg-[#2A629A] shadow-lg hover:shadow-indigo-500/50 justify-center items-center cursor-pointer text-white' >
          <span><FaList/></span>
        </div>
        ...
        <div className='flex justify-center items-center gap-8 relative'>
          ...
          <span className='text-[#928d8d] text-[14px] w-full font-normal'>{ userInfo.role }</span>
              </div>
