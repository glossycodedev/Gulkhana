...
const FeatureProducts = ({products}) => {
    ...
    {
        products.map((p,i) => <div key={i} className='border w-full group transition-all duration-500 hover:shadow-md rounded-md hover:-mt-3'>
            <div className='relative overflow-hidden '>
            
        {
            p.discount ? <div className='flex justify-center items-center absolute text-white w-[38px] h-[38px] rounded-full bg-[#B65278] font-semibold text-xs left-2 top-2'>{p.discount}% </div> : ''
        }

        <img className='sm:w-full w-full h-[240px] rounded-md' src={p.images[0]} alt="" />  
        ...
        <ul className='flex transition-all duration-700 -bottom-10 justify-center items-center gap-2 absolute w-full group-hover:bottom-3'>
            ...
            <Link to={`/product/details/${p.slug}`} className='w-[38px] h-[38px] cursor-pointer bg-white flex justify-center items-center rounded-full hover:bg-[#B65278] hover:text-white  transition-all'>
            <FaEye />
            </Link> 
